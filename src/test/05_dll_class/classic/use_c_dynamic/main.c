...
int main() {
...
#if defined(_WIN32) || defined(_WIN64)
  const char dllname[] =
      "MyCMake_test_05_dll_class_classic_gen" MYCMAKE_CONFIG_POSTFIX ".dll";
  ...
  fp_MyClass_Construct =
      (F_MyClass_Construct)GetProcAddress(dll, "MyClass_Construct");
  fp_MyClass_Destruct =
      (F_MyClass_Destruct)GetProcAddress(dll, "MyClass_Destruct");
  fp_MyClass_SayHello =
      (F_MyClass_SayHello)GetProcAddress(dll, "MyClass_SayHello");
#else
  const char soname[] =
      "./"
      "lib"
      "MyCMake_test_05_dll_class_classic_gen" MYCMAKE_CONFIG_POSTFIX ".so";
